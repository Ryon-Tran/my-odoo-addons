...
class ITServiceRequest(models.Model):
    ...
    ], string="Loại yêu cầu", default='onboarding', required=True)
...
"""
            ...
            if response.status_code in [200, 201]:
                self.state = 'done'
                _logger.info(f"IT Request {self.name} processed successfully")
            else:
                self.state = 'failed'
